...
export function registerNotificationRoutes(
  ...
  app.get<{ Params: { id: string } }>("/workspaces/:id/notifications", async (request, reply) => {
    ...
    return await listChannels(db, request.params.id);
  });
  ...
  app.post<{ Params: { id: string } }>("/workspaces/:id/notifications", async (request, reply) => {
    ...
    const channel = await upsertChannel(db, request.params.id, parsed.data);
  ...
  app.put<{ Params: { id: string; channelId: string } }>(
    ...
    async (request, reply) => {
      ...
      const existing = (await listChannels(db, request.params.id)).find((c) => c.id === request.params.channelId);
      if (!existing) return reply.status(404).send({ error: "not_found" });

      const updated = await upsertChannel(db, request.params.id, {
  ...
  app.delete<{ Params: { id: string; channelId: string } }>(
    ...
    async (request, reply) => {
      if (!await workspaceOr404(db, request.params.id, reply)) return reply;
      await deleteChannel(db, request.params.id, request.params.channelId);
  ...
  app.get<{ Params: { token: string } }>("/a/:token", async (request, reply) => {
    ...
    const result = await verifyAndBurn(db, request.params.token);
    if (!result.ok) {
    ...
    const draft = await getDraft(db, result.workspaceId, result.draftId);
    ...
    if (body.callback_query) {
      ...
      if (typeof data === "string") {
        ...
        if ((action === "approve" || action === "reject") && token) {
          const result = await verifyAndBurn(db, token);
