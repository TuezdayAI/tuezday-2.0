...
export function registerCampaignRoutes(app: FastifyInstance, db: Db): void {
  app.post<{ Params: { id: string } }>("/workspaces/:id/campaigns", async (request, reply) => {
    ...
    return reply.status(201).send(await createCampaign(db, request.params.id, parsed.data));
  });

  app.get<{ Params: { id: string } }>("/workspaces/:id/campaigns", async (request, reply) => {
    if (!await workspaceOr404(db, request.params.id, reply)) return reply;
    return await listCampaigns(db, request.params.id);
  });
  ...
  app.get<{ Params: { id: string; campaignId: string } }>(
    ...
    async (request, reply) => {
      ...
      return await getCampaignDetail(db, campaign);
    },
  );
  ...
  app.put<{ Params: { id: string; campaignId: string } }>(
    ...
    async (request, reply) => {
      ...
      const updated = await updateCampaign(db, request.params.id, request.params.campaignId, parsed.data);
