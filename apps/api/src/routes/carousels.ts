...
import { CarouselSourceError, generateCarousel } from "../services/carousels";
  ...
  app.post<{ Params: { id: string; draftId: string } }>(
    ...
    async (request, reply) => {
      ...
      try {
        ...
        // Same status/shape as the text-generation routes — the UI shows the
        // standard upgrade prompt, never an "add API key" prompt.
        if (err instanceof EntitlementError) {
          return reply.status(402).send({ error: "upgrade_required", key: err.key, limit: err.limit });
        }
        if (err instanceof CarouselSourceError) {
          if (err.message === "draft_not_found") {
            return reply.status(404).send({ error: "draft_not_found" });
          }
          return reply.status(409).send({ error: "source_not_approved", message: err.message });
        }
