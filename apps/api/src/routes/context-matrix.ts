...
export function registerContextMatrixRoutes(app: FastifyInstance, db: Db): void {
  app.get<{ Params: { id: string } }>("/workspaces/:id/context-matrix", async (request, reply) => {
    ...
    return await listMatrixCells(db, request.params.id);
  });
  ...
  app.put<{ Params: { id: string; taskType: string; docType: string } }>(
    ...
    async (request, reply) => {
      ...
      return await setMatrixCell(db, request.params.id, request.params.taskType, request.params.docType, parsed.data);
    },
  );
  ...
  app.delete<{ Params: { id: string; taskType: string; docType: string } }>(
    ...
    async (request, reply) => {
      ...
      return await resetMatrixCell(db, request.params.id, request.params.taskType, request.params.docType);
    },
  );
}
