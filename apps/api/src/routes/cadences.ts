...
async function validateRefs(
  ...
  if (refs.campaignId !== undefined && !await getCampaign(db, workspaceId, refs.campaignId)) {
    void reply.status(400).send({ error: "campaign_not_found" });
    return false;
  }
  if (refs.personaId != null && !await getPersona(db, workspaceId, refs.personaId)) {
    void reply.status(400).send({ error: "persona_not_found" });
    return false;
  }
  if (refs.connectionId !== undefined) {
    const connection = await getConnection(db, workspaceId, refs.connectionId);
    if (!connection) {
...
export function registerCadenceRoutes(
  ...
  app.post<{ Params: { id: string } }>("/workspaces/:id/cadences", async (request, reply) => {
    ...
    if (!cadenceInput.connectionId && cadenceInput.personaId) {
      const routed = await resolvePersonaSocialConnection(db, request.params.id, {
    ...
    if (!await validateRefs(db, request.params.id, { connectionId: cadenceInput.connectionId }, reply)) {
      return reply;
    }
    const resolvedCadenceInput = { ...cadenceInput, connectionId: cadenceInput.connectionId };
    return reply.status(201).send(await createCadence(db, request.params.id, resolvedCadenceInput));
  });

  app.get<{ Params: { id: string } }>("/workspaces/:id/cadences", async (request, reply) => {
    if (!await workspaceOr404(db, request.params.id, reply)) return reply;
    return await listCadences(db, request.params.id, Date.now());
  });
  ...
  app.get<{ Params: { id: string; cadenceId: string } }>(
    ...
    async (request, reply) => {
      if (!await workspaceOr404(db, request.params.id, reply)) return reply;
      const cadence = await getCadence(db, request.params.id, request.params.cadenceId);
      if (!cadence) return reply.status(404).send({ error: "cadence_not_found" });
      return await getCadenceDetail(db, request.params.id, cadence, Date.now());
    },
  );
  ...
  app.patch<{ Params: { id: string; cadenceId: string } }>(
    ...
    async (request, reply) => {
      if (!await workspaceOr404(db, request.params.id, reply)) return reply;
      const existing = await getCadence(db, request.params.id, request.params.cadenceId);
      ...
      if (!await validateRefs(db, request.params.id, parsed.data, reply)) return reply;
      return await updateCadence(db, request.params.id, request.params.cadenceId, parsed.data);
    },
  );
  ...
  app.delete<{ Params: { id: string; cadenceId: string } }>(
    ...
    async (request, reply) => {
      if (!await workspaceOr404(db, request.params.id, reply)) return reply;
      if (!await deleteCadence(db, request.params.id, request.params.cadenceId)) {
  ...
  app.post<{ Params: { id: string; cadenceId: string } }>(
    ...
    async (request, reply) => {
      ...
      const detail = await getCadenceDetail(db, request.params.id, cadence, Date.now());
  ...
  app.get<{ Params: { id: string }; Querystring: { from?: string; to?: string } }>(
    ...
    async (request, reply) => {
      ...
      return await buildCalendar(db, request.params.id, from, to);
    },
  );
}
