...
describe("R2REvidenceStore", () => {
  ...
  it("ingests via multipart raw_text, attaches to the collection, and returns the id", async () => {
    const { fetcher, calls } = capture({
      "/v3/documents": { status: 202, body: { results: { document_id: "doc-123" } } },
      "/v3/collections/col-1/documents/doc-123": { status: 200, body: {} },
    });
    const store = new R2REvidenceStore("http://r2r.test", fetcher);
    ...
    const ingest = calls.find((c) => c.url.endsWith("/v3/documents"))!;
    expect(ingest.method).toBe("POST");
    const form = ingest.body as FormData;
    ...
    const attach = calls.find((c) => c.url.endsWith("/v3/collections/col-1/documents/doc-123"))!;
    expect(attach.method).toBe("POST");
  });

  it("searches scoped to a collection and parses chunk results", async () => {
    const { fetcher, calls } = capture({
      "/v3/retrieval/search": {
        status: 200,
        body: {
    ...
    const store = new R2REvidenceStore("http://r2r.test", fetcher);
    const results = await store.search("memory layer", "col-1", 5);
    ...
    expect(body.search_settings.filters).toEqual({ collection_ids: { $overlap: ["col-1"] } });
    expect(body.search_settings.limit).toBe(5);
  });

  it("creates a collection and returns its id", async () => {
    const { fetcher, calls } = capture({
      "/v3/collections": { status: 201, body: { results: { id: "col-9" } } },
    });
    const store = new R2REvidenceStore("http://r2r.test", fetcher);
    const id = await store.createCollection("ws-1");
    expect(id).toBe("col-9");
    const call = calls.find((c) => c.url.endsWith("/v3/collections"))!;
    expect(call.method).toBe("POST");
    expect(JSON.parse(call.body as string)).toMatchObject({ name: "ws-1" });
  });
