...
export function ApprovalsQueue({
  ...
  workspaceId: string;
  /** Reports the authoritative pending-review count to the Review shell. */
  ...
  const load = useCallback(async () => {
    try {
      ...
      const draftList = (await dRes.json()) as Draft[];
      setDrafts(draftList);
      onPendingCount?.(draftList.filter((d) => d.state === "pending_review").length);
  ...
  function renderCard(d: Draft) {
    ...
    return (
      <PreviewCard
        ...
        workflowStatus={draftWorkflowStatus(d.state)}
    ...
    return (
      <div className={styles.detail}>
        <div className={styles.detailHead}>
          <WorkflowStatusBadge status={draftWorkflowStatus(d.state)} />
