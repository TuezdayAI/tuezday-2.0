...
9. **Gmail-only email-enabled:** `getEmailSafetySettings` currently returns `killSwitch: true` when no `workspaceEmailSenders` row exists — which blocks Gmail-only workspaces. Fix: when there is no Resend sender row **but the workspace has ≥1 connected mailbox**, treat email as enabled (`killSwitch: false`, a sane default cap). The kill switch still works whenever a sender row exists; the per-recipient permission + suppression checks are unchanged; the sequence must still be explicitly activated. (Exact call sites confirmed by recon.)
