...
- 2026-07-24 — Part 1 built (2 parallel agents: API + web). Contracts + `chat_sessions`/`chat_messages` schema + migration 0053. API: `services/chat.ts` (persistence), `services/copilot-tools.ts` (10-tool read-only registry = the whitelist), `services/copilot.ts` (`runCopilotTurn` bounded JSON tool-loop, `MAX_ITERS=6`, degrade-on-failure, `parseJsonObject`), `routes/chat.ts` (`registerChatRoutes` wired after `registerSignalRoutes`). Web: global copilot slide-over launched from the workspace sidebar (session switcher, thread, citation chips, composer, 404-degrade). 10 new API tests (scripted fake LLM, tool routing, citations, iteration cap, gateway/parse degradation, persistence, workspace scoping, read-only + no-mutation guardrails). Full suite green (1607) + typecheck clean. Awaiting founder acceptance + merge.
